...
### Added
- **LangGraph migration**: new `src/graph/` package with `ArticleState` TypedDict, LangChain `@tool` wrappers, an AI review sub-graph (`edit → validate → retry`), and a full `extract → ai-review → upload` pipeline graph with a `human_review` interrupt and config-driven auto-confirm. (`src/graph/state.py`, `src/graph/tools.py`, `src/graph/graph.py`)
