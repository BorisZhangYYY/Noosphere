...
export function BatchForm({ onCreated }: { onCreated: (job: BatchJob) => void }) {
  ...
  return (
    <form className="batch-form" onSubmit={e => { e.preventDefault(); create.mutate({ urls, mode, language, collectionId }); }}>
      <label>{zh ? "每行一个链接（最多 100 条）" : "One URL per line (up to 100)"}
        <textarea rows={6} value={text} onChange={e => { setText(e.target.value); preview.reset(); }} placeholder="https://mp.weixin.qq.com/s/…" />
      </label>
      <label>{zh ? "导入文本文件" : "Import text file"}
      ...
      <div className="discovery-toolbar">
        <button type="button" disabled={!urls.length || urls.length > 100 || preview.isPending} onClick={() => preview.mutate(urls)}>{zh ? "检查链接" : "Check URLs"}</button>
        <button type="submit" disabled={!urls.length || urls.length > 100 || create.isPending}>{zh ? "开始处理" : "Start batch"}</button>
      </div>
